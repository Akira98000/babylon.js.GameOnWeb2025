...
import { setupGamepadControls } from "./gamepadControls.js";
...
export function setupControls(scene, hero, animations, camera, canvas) {
    // Essayer d'abord les contrôles de manette de jeu
    // Le contrôleur de manette sera utilisé automatiquement si une manette est connectée,
    // quelle que soit la plateforme (mobile ou desktop)
    const gamepadControls = setupGamepadControls(scene, hero, animations, camera);
    
    // Si c'est un appareil mobile, configurer aussi les contrôles tactiles comme fallback
    if (isMobileDevice()) {
        const mobileControls = setupJoystickControls(scene, hero, animations, camera);
        scene.metadata.mobileControls = mobileControls;
    }
    
    // Pour les ordinateurs de bureau, configurer les contrôles clavier/souris standard
    if (!isMobileDevice()) {
        const inputMap = {};
    ...
    // Retourner les contrôles gamepad par défaut
